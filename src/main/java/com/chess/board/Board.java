            ...
            for(int j = 0; j < boardSquares[i].length; j++) {
                if (boardSquares[i][j].isOccupied()) {
                    ...
                    if(piece.getPieceColor() == PieceColor.BLACK) {
                        System.out.print(Chalk.on(String.valueOf(piece.getName().charAt(0))).bgGreen().black().bold() + " ");
                    } else {
                        System.out.print(Chalk.on(String.valueOf(piece.getName().charAt(0))).bgGreen().white().bold() + " ");
                    }
                } else {
                    // empty square
                    System.out.print("- ");
